...
const deleteUser = async (req, res) => {
  const { id } = req.params;
  const deleteUser = async => commandHandler.deleteUser(id);
  const sendResponse = async (result) => {
    result.err
      ? wrapper.response(res, "fail", result, "Delete User Cancel", httpError.NOT_FOUND)
      : wrapper.response(res, "success", result, "Delete User", http.OK);
  };
  sendResponse(await deleteUser())
} 
